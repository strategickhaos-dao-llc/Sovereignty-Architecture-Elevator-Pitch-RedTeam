...
"""
OpenAI ChatGPT connector using official export API.
"""
...
class OpenAIConnector:
    """Connector for OpenAI ChatGPT exports using delegated OAuth2."""
    
    EXPORT_API_URL = "https://api.openai.com/v1/chat/exports"
    ...
    def request_export(self) -> str:
        """
        Request an export (for providers that use async export generation).
        
        Returns:
            Export request ID
        """
        # OpenAI may implement async exports in the future
        headers = {
            "Authorization": f"Bearer {self.api_key}",
            "Content-Type": "application/json"
        }
    ...
    def check_export_status(self, export_id: str) -> Dict[str, Any]:
        """
        Check status of async export request.
        ...
        response = requests.get(
            f"{self.EXPORT_API_URL}/{export_id}",
            headers=headers
        )
        response.raise_for_status()

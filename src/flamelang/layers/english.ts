...
import { EnglishLayer, EnglishLayerExtract } from '../types';

export class EnglishLayerProcessor {
  /**
   * Extract semantic intent from English text
   */
  process(input: string): EnglishLayer {
   ...
   */
  private extractSemanticIntent(input: string): EnglishLayerExtract {
  ...
  private genericExtraction(input: string): EnglishLayerExtract {
    // Simple heuristic extraction
    const words = input.toLowerCase().split(/\s+/);
    
    // Find chart type
    let subject = "chart";
    if (words.includes("horizontal")) subject = "horizontal bar chart";
    else if (words.includes("vertical")) subject = "vertical bar chart";
    
    // Find condition keywords
    let condition = "unspecified";
    if (input.includes("long") && input.includes("label")) condition = "category labels are long";
    else if (input.includes("many categor")) condition = "many categories";
    else if (input.includes("negative") || input.includes("profit")) condition = "negative values";
    else if (input.includes("floor") || input.includes("building")) condition = "building floors";
    
    // Determine claim
    let claim = "preferable";
    if (input.includes("better")) claim = "better";
    if (input.includes("should")) claim = "should use";

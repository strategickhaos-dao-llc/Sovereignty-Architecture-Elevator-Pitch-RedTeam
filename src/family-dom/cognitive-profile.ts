...
export interface CognitiveLayer {
  mythic: string[];       // Mythological/narrative references
  emotional: string[];    // Emotional markers and patterns
  architectural: string[]; // System/structural concepts
  symbolic: string[];     // Symbol and metaphor usage
  technical: string[];    // Technical terminology and concepts
  kinetic: string[];      // Action-based metaphors
}
...
export interface CognitiveProfile {
  ...
  // Cognitive architecture preferences
  cognitiveStyle: CommunicationStyle;
  
  // Tracked layers and patterns
  layers: CognitiveLayer;
  
  // Long-term context memory
  contextMemory: ContextMemoryEntry[];
  
  // Project and narrative tracking
...
export interface NarrativeThread {
  ...
  symbols: string[];
  references: string[];
  continuity: number; // How many sessions this thread spans
}
...
export class CognitiveProfileManager {
  ...
  private defaultProfile: Partial<CognitiveProfile>;
   ...
   */
  getOrCreateProfile(userId: string, displayName?: string): CognitiveProfile {
   ...
   */
  createProfile(userId: string, displayName: string): CognitiveProfile {
    const now = new Date();
   ...
   */
  updateLayers(userId: string, layers: Partial<CognitiveLayer>): void {
    const profile = this.profiles.get(userId);
    if (!profile) return;

    for (const [layer, values] of Object.entries(layers)) {
      const key = layer as keyof CognitiveLayer;
   ...
   */
  addContextMemory(userId: string, content: string, layers: Partial<CognitiveLayer>, significance: number): void {
    const profile = this.profiles.get(userId);
    if (!profile) return;
   ...
   */
  updateCommunicationStyle(userId: string, updates: Partial<CommunicationStyle>): void {
    const profile = this.profiles.get(userId);
    if (!profile) return;
   ...
   */
  trackProject(userId: string, project: Omit<ProjectContext, 'id' | 'lastInteraction'>): void {
    const profile = this.profiles.get(userId);
    if (!profile) return;
   ...
   */
  recordIntentPattern(userId: string, pattern: string, resolvedTo: string): void {
    const profile = this.profiles.get(userId);
    if (!profile) return;
   ...
   */
  getRelevantContext(userId: string, currentInput: string): {
    recentMemory: ContextMemoryEntry[];
    activeProjects: ProjectContext[];
    relatedPatterns: IntentPattern[];
  } {
    const profile = this.profiles.get(userId);
    if (!profile) {
   ...
   */
  serializeProfile(userId: string): string | null {
    const profile = this.profiles.get(userId);
   ...
   */
  loadProfile(serialized: string): CognitiveProfile | null {
    try {
   ...
   */
  getAllProfiles(): CognitiveProfile[] {
   ...
   */
  getCognitiveSignature(userId: string): {
    score: number;

...
export interface IntentMetadata {
  hasParallelMetaphors: boolean;
  hasRecursiveIntent: boolean;
  hasMultiAgentReferences: boolean;
  hasTechnicalOverlay: boolean;
  hasMythicOverlay: boolean;
  hasMetaCommunication: boolean;
  hasHumor: boolean;
  hasSelfReference: boolean;
  hasCodeLoreBlending: boolean;
...
export class IntentParser {
   ...
   */
  parse(input: string, userId?: string): ParsedIntent {
   ...
   */
  private extractLayers(input: string): Partial<CognitiveLayer> {
    const layers: Partial<CognitiveLayer> = {};
   ...
   */
  private matchPatterns(input: string, patterns: RegExp[]): string[] {
   ...
   */
  private extractMetadata(input: string): IntentMetadata {
    return {
   ...
   */
  private detectParallelMetaphors(input: string): boolean {
   ...
   */
  private detectRecursiveIntent(input: string): boolean {
   ...
   */
  private detectCodeLoreBlending(input: string): boolean {
   ...
   */
  private countContextSwitches(input: string): number {
    const layers = [
   ...
   */
  private calculateConfidence(layers: Partial<CognitiveLayer>, metadata: IntentMetadata): number {
    ...
    if (metadata.hasMultiAgentReferences) confidence += 0.05;
    if (metadata.hasTechnicalOverlay) confidence += 0.05;
    if (metadata.hasRecursiveIntent) confidence += 0.03;
    if (metadata.hasMetaCommunication) confidence += 0.03;

    // High context switching might reduce confidence
    if (metadata.contextSwitchCount > 4) confidence -= 0.05;
   ...
   */
  private calculateSymbolicThroughput(layers: Partial<CognitiveLayer>): 'low' | 'medium' | 'high' | 'extreme' {
   ...
   */
  generateResponseStrategy(parsed: ParsedIntent, profile?: CognitiveProfile): ResponseStrategy {
  ...
  private determineTone(parsed: ParsedIntent, profile?: CognitiveProfile): string {
    if (parsed.metadata.hasHumor) return 'playful';
    if (parsed.layers.mythic && parsed.layers.mythic.length > 0) return 'epic';
    if (parsed.layers.emotional && parsed.layers.emotional.length > 0) return 'empathetic';
    if (parsed.layers.technical && parsed.layers.technical.length > 2) return 'precise';
    return 'balanced';
  }

  private determineDepth(parsed: ParsedIntent, profile?: CognitiveProfile): 'shallow' | 'medium' | 'deep' | 'fullstack' {
    if (profile?.cognitiveStyle.symbolicThroughput === 'extreme') return 'fullstack';
    if (parsed.metadata.contextSwitchCount >= 4) return 'deep';
    if (parsed.metadata.contextSwitchCount >= 2) return 'medium';
    return 'shallow';
  }

  private selectLayersToMatch(parsed: ParsedIntent, profile?: CognitiveProfile): (keyof CognitiveLayer)[] {
    const layers: (keyof CognitiveLayer)[] = [];
    
    if (parsed.layers.mythic) layers.push('mythic');
    if (parsed.layers.emotional) layers.push('emotional');
    if (parsed.layers.architectural) layers.push('architectural');
    if (parsed.layers.symbolic) layers.push('symbolic');
    if (parsed.layers.technical) layers.push('technical');
    if (parsed.layers.kinetic) layers.push('kinetic');
...
export interface ResponseStrategy {
  ...
  layersToMatch: (keyof CognitiveLayer)[];
  includeMetaReflection: boolean;
  includeNarrativeFraming: boolean;

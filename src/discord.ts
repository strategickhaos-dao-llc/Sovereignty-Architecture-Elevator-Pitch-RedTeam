...
export async function registerCommands(token: string, appId: string) {
  const cmds = [
    ...
    new SlashCommandBuilder().setName("fix").setDescription("Run automated remediation for security issues")
      .addStringOption(o => o.setName("issue").setRequired(true).setDescription("Issue type to fix").addChoices(
        { name: "RBAC", value: "rbac" },
        { name: "Network Security", value: "nsg" },
        { name: "Azure Policy", value: "policy" },

...
export async function registerCommands(token: string, appId: string) {
  const cmds = [
    ...
    new SlashCommandBuilder().setName("snapshot").setDescription("Create system state snapshot")
      .addStringOption(o => o.setName("id").setDescription("Optional snapshot ID").setRequired(false))
  ].map(c => c.toJSON());

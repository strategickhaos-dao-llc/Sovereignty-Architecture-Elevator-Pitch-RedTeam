...
const LAYER_CAPABILITIES: Record<number, string[]> = {
...
async function processTask(task: Task): Promise<Finding> {
...
async function generateFinding(task: Task): Promise<Finding> {
...
function getLayerPrompt(layerId: number, task: Task): string {
...
function getLayerTemperature(layerId: number): number {
...
async function reportFinding(taskId: string, finding: Finding): Promise<void> {
  try {
...
async function calculateMove(position: string, opponentMove: string): Promise<{ notation: string; reasoning: string }> {
...
function isHarmonicCompatible(freq1: number, freq2: number): boolean {
...
async function sendHeartbeat(): Promise<void> {
  try {
    ...
    if (response.ok) {
      const data = await response.json() as { nextTask?: Task };

...
import { BibliographyClient, BibliographyResult } from './bibliography.js';
...
const searchCache = new Map<string, { results: BibliographyResult[]; timestamp: number }>();
...
app.get('/paper/:id', async (req, res) => {
  ...
  try {
    const results = await bibliographyClient.search({
      query: id,
      sources: [source as any || 'semantic_scholar'],
      maxResults: 1
    });
...
app.post('/batch-search', async (req, res) => {
  ...
  try {
    const batchResults = await bibliographyClient.batchSearch(
      queries,
      sources || ['arxiv', 'semantic_scholar']
    );

    const response: { query: string; resultsCount: number; results: BibliographyResult[] }[] = [];

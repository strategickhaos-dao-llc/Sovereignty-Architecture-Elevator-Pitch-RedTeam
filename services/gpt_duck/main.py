# gpt_duck agent - Pattern Analyst
...
async def handle_deliberation(msg: dict):
    """Agent-specific logic: analyze patterns."""
    decision = {"role": "pattern_analyst", "recommendation": "Approve infra topology"}
...
async def trigger_deliberation(topic: str = "board.deliberate"):
    ...
    await broker.publish({"trigger": "manual", "source": "gpt_duck"}, topic)
...
async def health():
    ...
    return {"status": "healthy", "role": os.getenv("ROLE", "pattern_analyst")}

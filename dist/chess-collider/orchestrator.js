...
const CIRCLE_OF_FIFTHS_FREQUENCIES = {
    1: 440.00, // A4 - Root
    2: 493.88, // B4
    3: 554.37, // C#5
    4: 622.25, // D#5
...
// HELPER FUNCTIONS
// =============================================================================
function getNextTaskForAgent(agent) {
    // Find queued research tasks for this agent's layer
    const task = researchQueue.find(t => t.status === 'in_progress' &&
...
function getLayerInstruction(layerId, topic) {
    const instructions = {
...
function broadcastToLayer(layerId, message) {
    const layer = agentRegistry.get(layerId);
    if (!layer)
        return;
...
async function processResearchTask(task) {
...
function getMusicalInterval(semitones) {
    const intervals = {
        0: 'Unison',
        1: 'Minor 2nd',
        2: 'Major 2nd',

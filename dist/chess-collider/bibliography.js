...
export class BibliographyClient {
    ...
    semanticScholarEndpoint = 'https://api.semanticscholar.org/graph/v1';
    ...
    async search(params) {
        ...
        for (const sourceResults of allResults) {
            for (const result of sourceResults) {
                ...
                const isDuplicate = results.some(r => this.calculateTitleSimilarity(r.title, result.title) > 0.9);
                if (!isDuplicate) {
                    results.push(result);
                }
            }
    ...
    // =============================================================================
    async searchGoogleScholar(params) {
    ...
    // =============================================================================
    async searchGovDatabases(params) {
        const { query, maxResults = 20 } = params;
        const results = [];
        // Data.gov search
        try {
            ...
            if (response.ok) {
                ...
                for (const dataset of data.result.results || []) {
                    results.push({
                        ...
                        categories: dataset.tags?.map((t) => t.name) || [],
                        relevanceScore: 0.6
                    });
                }
            }
    ...
    // BULK OPERATIONS
    // =============================================================================
    async batchSearch(queries, sources) {

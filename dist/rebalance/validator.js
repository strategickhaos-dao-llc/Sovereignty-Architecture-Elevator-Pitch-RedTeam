 ...
 */
import { ExecutionPlanSchema, PortfolioConfigSchema, PaycheckEventSchema, PortfolioSnapshotSchema, } from './types.js';
/**
 ...
 */
function validateSchemas(input) {
    const errors = [];
 ...
 */
function validateMath(paycheck, config, plan) {
    const errors = [];
    ...
    if (Math.abs(plan.summary.investable_amount_usd - expectedInvestable) > tolerance) {
        errors.push(`Investable amount mismatch: expected ${expectedInvestable.toFixed(2)}, got ${plan.summary.investable_amount_usd.toFixed(2)}`);
    }
    // Validate treasury amount
    if (Math.abs(plan.summary.treasury_amount_usd - expectedTreasury) > tolerance) {
        errors.push(`Treasury amount mismatch: expected ${expectedTreasury.toFixed(2)}, got ${plan.summary.treasury_amount_usd.toFixed(2)}`);
    }
    // Validate rebalance amount
    if (Math.abs(plan.summary.rebalance_amount_usd - expectedRebalance) > tolerance) {
        errors.push(`Rebalance amount mismatch: expected ${expectedRebalance.toFixed(2)}, got ${plan.summary.rebalance_amount_usd.toFixed(2)}`);
    }
    ...
    if (Math.abs(plan.treasury_transfer.amount_usd - plan.summary.treasury_amount_usd) > tolerance) {
        errors.push(`Treasury transfer amount ${plan.treasury_transfer.amount_usd.toFixed(2)} does not match summary ${plan.summary.treasury_amount_usd.toFixed(2)}`);
    }
    ...
    if (totalTradeValue > plan.summary.rebalance_amount_usd + tolerance) {
        errors.push(`Total trade value ${totalTradeValue.toFixed(2)} exceeds rebalance amount ${plan.summary.rebalance_amount_usd.toFixed(2)}`);
    }
    return errors;
}
/**
 ...
 */
function validateConstraints(config, snapshot, plan) {
    const errors = [];
        ...
        for (const trade of plan.trades) {
            if (trade.action === 'SELL') {
                ...
                if (trade.shares > currentShares) {
                    errors.push(`Trade would result in short position: selling ${trade.shares} shares of ${trade.symbol}, but only have ${currentShares}`);
                }
            }
        }
    }
    ...
    if (config.constraints.execution.forbid_margin) {
        ...
        if (buyValue > availableForBuying) {
            errors.push(`Trades require margin: buying ${buyValue.toFixed(2)} but only ${availableForBuying.toFixed(2)} available`);
        }
    }
    ...
    for (const trade of plan.trades) {
        ...
        if (trade.approx_value_usd < minSize) {
            errors.push(`Trade for ${trade.symbol} ($${trade.approx_value_usd.toFixed(2)}) is below minimum trade size ($${minSize})`);
        }
    }
    // Check cash buffer requirement
    const requiredBuffer = snapshot.total_portfolio_value_usd * (config.constraints.liquidity.keep_cash_buffer_pct / 100);
    const netCashChange = plan.trades
        .filter((t) => t.action === 'SELL')
        .reduce((sum, t) => sum + t.approx_value_usd, 0) -
        plan.trades
            .filter((t) => t.action === 'BUY')
    ...
    if (projectedCash < requiredBuffer) {
        errors.push(`Projected cash ${projectedCash.toFixed(2)} would be below required buffer ${requiredBuffer.toFixed(2)}`);
    }
    return errors;
}
/**
 ...
 */
function validateTradeDirection(config, snapshot, plan) {
    const warnings = [];
    ...
    for (const trade of plan.trades) {
        ...
        if (trade.action === 'BUY' && !isUnderweight) {
            warnings.push(`Buying ${trade.symbol} but it's already at or above target (${currentWeight.toFixed(1)}% vs ${targetWeight}% target)`);
        }
        if (trade.action === 'SELL' && isUnderweight) {
            warnings.push(`Selling ${trade.symbol} but it's below target (${currentWeight.toFixed(1)}% vs ${targetWeight}% target)`);
        }
    }
    return warnings;
}
 ...
 */
function validateHardGuards(snapshot, plan) {
    const errors = [];
    // No negative shares
    for (const trade of plan.trades) {
        if (trade.shares < 0) {
 ...
 */
export function generateDryRunSummary(input, validation) {
    ...
    for (const trade of plan.trades) {
        lines.push(`| ${trade.action} | ${trade.symbol} | ${trade.shares.toFixed(4)} | $${trade.approx_value_usd.toFixed(2)} | ${trade.reason} |`);
    }
    lines.push('');
    ...
    lines.push('---');
    lines.push('');
    lines.push(`**Portfolio Value:** $${snapshot.total_portfolio_value_usd.toFixed(2)} | **Cash Available:** $${snapshot.cash_available_usd.toFixed(2)}`);
    return lines.join('\n');
}
/**
 * Parse and validate a raw Grok JSON response
 */
export function parseGrokResponse(rawJson) {
    try {

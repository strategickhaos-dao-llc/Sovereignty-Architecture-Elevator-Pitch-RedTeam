 ...
 */
export function isInSunshineMode(sunshineHours, timezone = 'UTC') {
 ...
 */
export function getCurrentCircadianMode(spec) {
    const rhythm = spec.circadianRhythm;
    if (!rhythm?.enabled)
        return 'sunshine';
    return isInSunshineMode(rhythm.sunshineHours || DEFAULTS.circadianRhythm.sunshineHours, rhythm.timezone || DEFAULTS.circadianRhythm.timezone) ? 'sunshine' : 'moonlight';
}
/**
 * Get target replica count based on circadian mode
 */
export function getTargetReplicas(spec) {
    const rhythm = spec.circadianRhythm;
    if (!rhythm?.enabled) {
 ...
 */
export function determinePhase(spec, activeThreats, previousPhase) {
 ...
 */
export function isImageAllowed(image, allowlist) {
 ...
 */
export function isNamespaceTrusted(namespace, trustedNamespaces) {
 ...
 * Update status phase
 */
export function updatePhase(status, phase, reason, message) {
 ...
 */
export function createSignatureFromEvent(event, vector) {
    return {
        id: `sig-${event.id}`,
        vector,
        type: event.type,
 ...
 */
export function calculateMetrics(status, spec) {
    return {

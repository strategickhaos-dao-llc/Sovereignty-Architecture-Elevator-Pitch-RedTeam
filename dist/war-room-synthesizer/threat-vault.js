...
export class ThreatVault {
    ...
    generateThreatMatrix(options) {
        ...
        const mitreTactics = [
            { id: 'TA0001', name: 'Initial Access', findings: [] },
            { id: 'TA0002', name: 'Execution', findings: [] },
            { id: 'TA0003', name: 'Persistence', findings: [] },
            { id: 'TA0004', name: 'Privilege Escalation', findings: [] },
            { id: 'TA0005', name: 'Defense Evasion', findings: [] },
            { id: 'TA0006', name: 'Credential Access', findings: [] },
            { id: 'TA0007', name: 'Discovery', findings: [] },
            { id: 'TA0008', name: 'Lateral Movement', findings: [] },
            { id: 'TA0009', name: 'Collection', findings: [] },
            { id: 'TA0011', name: 'Command and Control', findings: [] }
        ];
     ...
     */
    generateNodeTrustLevels(options) {
     ...
     */
    generateHardeningPlaybook(options) {
        return {
     ...
     */
    generateCounterAttackStrategies(options) {
        return {
    ...
    async generateMarkdownFiles() {
        ...
        fs.writeFileSync(path.join(this.vaultPath, 'Threat Matrix', 'Overview.md'), threatMatrixContent);
        // Node trust levels
        const nodeTrustContent = this.generateNodeTrustMarkdown();
        fs.writeFileSync(path.join(this.vaultPath, 'Node Trust', 'Assessment.md'), nodeTrustContent);
        // Hardening playbook
        const hardeningContent = this.generateHardeningMarkdown();
        fs.writeFileSync(path.join(this.vaultPath, 'Hardening', 'Playbook.md'), hardeningContent);
        // Counter-attack strategies
        const counterAttackContent = this.generateCounterAttackMarkdown();
        fs.writeFileSync(path.join(this.vaultPath, 'Counter-Attack', 'Strategies.md'), counterAttackContent);
    }
    /**

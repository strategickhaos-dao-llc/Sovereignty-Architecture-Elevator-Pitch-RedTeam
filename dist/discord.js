...
export async function registerCommands(token, appId) {
    const cmds = [
        ...
        new SlashCommandBuilder().setName("progress").setDescription("Show autonomous progress & activation proof")
            .addStringOption(o => o.setName("view").setRequired(false).addChoices({ name: "full", value: "full" }, { name: "quick", value: "quick" }, { name: "validate", value: "validate" }, { name: "emergency", value: "emergency" }))
    ].map(c => c.toJSON());

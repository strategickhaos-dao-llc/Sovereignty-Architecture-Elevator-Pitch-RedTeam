...
export class OutputSanitizer extends BaseAntibody {
    ...
    async determineHealingActions(health) {
        const actions = [];
        const rejectRate = health.metrics.reject_rate;
        if (rejectRate > 50) {
     ...
     */
    queueOutput(id, type, content, language) {
     ...
     */
    async validateCode(output, result) {
        if (!this.sanitizerConfig.ast_validation)
            return;
    ...
    async validateJavaScript(filePath, result) {
        try {
    ...
    async validatePython(filePath, result) {
        try {
     ...
     */
    async validateDocker(output, result) {
        if (!this.sanitizerConfig.docker_lint)
            return;
        // Create temp file for validation
        const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'sanitizer-'));
        ...
        try {
            ...
            const { stdout, stderr } = await execAsync(`hadolint ${tempFile} 2>&1 || echo "HADOLINT_ERROR"`);
    ...
    basicDockerValidation(content, result) {
     ...
     */
    async validateConfig(output, result) {
     ...
     */
    async sandboxTest(output, result) {

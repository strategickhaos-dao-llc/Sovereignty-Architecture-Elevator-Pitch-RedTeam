...
client.on("interactionCreate", async (i) => {
    ...
    try {
        ...
        if (i.commandName === "status") {
            ...
            const r = await fetchWithTimeout(`${cfg.control_api.base_url}/status/${svc}`, { headers: authHeader }, 5000).then(r => r.json());
            ...
            const r = await fetchWithTimeout(`${cfg.control_api.base_url}/logs/${svc}?tail=${tail}`, { headers: authHeader }, 8000).then(r => r.text());
            await i.editReply({ content: "```\n" + r.slice(0, 1800) + "\n```" });
        }
        else if (i.commandName === "deploy") {
            const envName = i.options.getString("env", true);
            const tag = i.options.getString("tag", true);
            const r = await fetchWithTimeout(`${cfg.control_api.base_url}/deploy`, {
                method: "POST",
            ...
            const r = await fetchWithTimeout(`${cfg.control_api.base_url}/scale`, {
                method: "POST",
                headers: { "Content-Type": "application/json", ...authHeader },
        ...
        await i.editReply({ content: errorMsg }).catch(() => i.followUp({ content: errorMsg, ephemeral: true }));
    }
});
client.login(token);

...
import { createFamilyDOM } from "./family-dom/index.js";
const cfg = loadConfig();
const token = env("DISCORD_TOKEN");
const appId = env("APP_ID", false) || cfg.discord?.bot?.app_id || "";
const client = new Client({ intents: [GatewayIntentBits.Guilds] });
// FamilyDOM instances per user
const familyDomInstances = new Map();
function getFamilyDOM(userId, displayName) {
...
client.on("interactionCreate", async (i) => {
    ...
    try {
        ...
        // FamilyDOM commands
            ...
            await i.reply({
                embeds: [cognitiveEmbed(`🧠 Cognitive Profile: ${i.user.displayName}`, "Your FamilyDOM cognitive architecture status. Interact more to enhance your profile!", fields)]
            });
        }
            ...
            await i.reply({
                embeds: [cognitiveEmbed(`📋 Cognitive Analysis Report: ${report.displayName}`, "Full-stack cognitive reasoning analysis of your communication patterns.", fields)]
            });
        }
            ...
            const changes = [];
            if (throughput !== null) {
                updates.symbolicThroughput = throughput;
            ...
            if (Object.keys(updates).length > 0) {
                dom.updateStyle(updates);
                await i.reply({
                    embeds: [cognitiveEmbed("✅ Cognitive Style Updated", "Your FamilyDOM preferences have been configured.", [{ name: "Changes Applied", value: changes.join("\n") }])]
                });
            }
            else {
            ...
            await i.reply({
                embeds: [cognitiveEmbed("📂 Project Tracked", `"${name}" is now tracked for cognitive continuity.`, [

...
export class ContextFrameManager {
    frames = new Map();
    sessionFrames = new Map(); // sessionId -> frameId
    /**
     * Get or create a context frame for a user session
     */
    getOrCreateFrame(userId, sessionId) {
     ...
     */
    addMessage(frameId, role, content, parseIntent = true) {
        ...
        if (parseIntent && role === 'user') {
            ...
            profileManager.addContextMemory(frame.userId, content, message.parsedIntent.layers, message.significance);
        }
     ...
     */
    updateReasoningThreads(frame, message) {
        if (!message.parsedIntent)
            return;
        const intent = message.parsedIntent;
        const layers = Object.keys(intent.layers);
        // Find or create relevant reasoning thread
        let matchingThread = frame.reasoningThreads.find(t => t.status === 'active' &&
    ...
    updateMetadata(frame) {
        ...
        for (const [layer, values] of Object.entries(frame.activeLayers)) {
            layerCounts[layer] = values.length;
        }
        ...
        frame.metadata.continuityScore = Math.min(1, (contextRelevance.recentMemory.length * 0.1) +
     ...
     */
    setContinuityMarker(frameId, key, value, source = 'inferred', expiresIn // milliseconds
    ) {
        const frame = this.frames.get(frameId);
        if (!frame)
            return;
     ...
     */
    getContinuityMarker(frameId, key) {
        const frame = this.frames.get(frameId);
        if (!frame)
            return undefined;
        const marker = frame.continuityMarkers.find(m => m.key === key &&
     ...
     */
    branchThread(frameId, parentThreadId, newTheme) {
        const frame = this.frames.get(frameId);
        if (!frame)
            return null;
     ...
     */
    getReasoningPath(frameId, threadId) {
        const frame = this.frames.get(frameId);
        if (!frame)
            return [];
        const thread = frame.reasoningThreads.find(t => t.id === threadId);
